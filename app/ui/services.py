...
from core.inspect import profile_collection
...
def profile_many(
    settings: Settings, names: list[str], sample: int, schema: str
...
def profile_one(settings: Settings, name: str, sample: int, schema: str) -> dict:
    mongo = mongo_client(settings.mongo)
    try:
        mongo.connect()
